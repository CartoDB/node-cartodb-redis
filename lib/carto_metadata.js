...
module.exports = function(redis_opts) {
        ...
        if (_.isUndefined(stat_tag)) {
            Step (
                function incrementGlobal() {
                    ...
                    that.redisCmd(me.user_metadata_db, 'ZINCRBY', [redisKey, 1, mapViewKey], this);
                },
    ...
    me.getOAuthHash = function(oAuthAccessKey, callback) {
        var redisKey = _.template(this.oauth_user_key, {oauth_access_key: oAuthAccessKey});
    ...
    me.redisCmd = function(db, redisFunc, redisArgs, callback) {
        genericCommand(db, function(client, done, commands) {
            if (!!commands) {
    ...
    me.redisMultiCmd = function (db, redisCmdsAndArgs, callback) {
        genericCommand(db, function(client, done, commands) {
            if (!!commands) {
                _.each(redisCmdsAndArgs, function(cmd) {
                    commands.push(cmd[0]);
                });
            }
        ...
        Step(
            ...
            function executeQuery(err, client) {
                ...
                if (logSlowQueries) {
                    ...
                    execQueryFunc(client, this, commands);
                } else {
                    execQueryFunc(client, this);
                }
            },
