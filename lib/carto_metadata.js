...
module.exports = function(redis_opts) {
    ...
    me.getMultipleUserParams = function (username, fields, callback) {
        const redisKey = this.user_key({ username: username });

        this.mRetrieve(this.user_metadata_db, redisKey, fields, function (err, dbValues) {
            if (err) {
                ...
                fields.forEach((field, pos) => {
                    element[field] = dbValues[pos];
                    found = found || !!dbValues[pos];
                });
    ...
    const GRANTS_THIS_API = `grants_${options.apiName}`;
    ...
    function transformApiKeyArrayToObject(apiKeyArray) {
        const element = {};
        apiKeyFields.forEach((field, pos) => {
            if (field === GRANTS_THIS_API) {
                field = 'grants_access';
            }
            element[field] = apiKeyArray[pos];
        });
        return element;
    }
    ...
    me.getApiKey = function ({ username, apiKeyToken }, callback) {
        ...
        this.mRetrieve(this.user_metadata_db, redisKey, apiKeyFields, function (err, apiKeyArray) {
            if (err) {
                return callback(err, null);
            } else {
                const apiKey = transformApiKeyArrayToObject(apiKeyArray);
